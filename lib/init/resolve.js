...
exports.default = async (ctx) => {
    ...
    if (value) {
        core_1.config.registry = 'https://github.91chi.fun/https://github.com/{owner}/{name}/archive/refs/heads/{branch}.zip';
    }
    ...
    if (ctx.options.offline != null && ctx.options.offline) {
        if (exists) {
            return console.log(`将使用缓存中的模板：${core_1.file.tildify(ctx.src)}`);
        }
        console.log(`未发现缓存模板：${core_1.file.tildify(ctx.src)}`);
    }
    // 当缓存存在时，清除缓存
    ...
    // 开始转圈...
    const spinner = ora_1.default('下载模板中...').start();
    try {
        // 使用http模板下载zip文件，temp是一个zip文件的绝对路径
        const temp = await core_1.http.downolad(url);
        spinner.text = '开始解压...';
        // 解压zip文件到目标项目中
